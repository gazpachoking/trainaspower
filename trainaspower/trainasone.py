...
def get_workout(
    workout_url: str, date: datetime.date, config: models.Config
) -> models.Workout:
    workout_json_url = workout_url.replace(
        "plannedWorkout?", "plannedWorkoutDownload?sourceFormat=GARMIN_TRAINING&"
    )
    r = tao_session.get(
        workout_json_url, headers={"Content-Type": "application/json; charset=utf-8"}
    )
    ...
    try:
        ...
        w.steps = list(convert_steps(steps, config, "Perceived Effort" in name))
        return w
    except Exception as exc:
        raise FindWorkoutException(
...
def convert_steps(
...
) -> Generator[models.Step, None, None]:
    ...
    for step in steps:
        if step["type"] == "WorkoutRepeatStep":
            ...
            repeat_steps = list(convert_steps(step["steps"], config, perceived_effort))
            ...
            if step["targetType"] not in valid_target_types:
                raise ValueError(
            ...
            if step["durationType"] == "DISTANCE":
                out_step.length = round(step["durationValue"]) * models.meter
